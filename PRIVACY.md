...
- **Stop auto-extraction**: edit `~/.skills_data/soul-archive/config.json`, set `auto_extract`, `auto_reflect`, and `auto_context_inject` to `false`. After this, nothing is captured unless you explicitly trigger it.
- **Disable specific axes**: in the same `config.json`, flip any axis under `extract_dimensions` to `false`.
- **Delete everything**: `rm -rf ~/.skills_data/soul-archive`. There is no other copy.
